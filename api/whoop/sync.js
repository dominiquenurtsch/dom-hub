...
async function refreshTokens(tokens) {
  ...
  await kvSet('dom_whoop_tokens', JSON.stringify(updated));
  return updated;
}
...
module.exports = async function handler(req, res) {
  try {
    let tokens = await kvGet('dom_whoop_tokens');
    if (!tokens) {
    ...
    const history = await kvGet('dom_whoop_history') || {};
    ...
    // Save history
    await kvSet('dom_whoop_history', JSON.stringify(history));
    ...
    await kvSet('dom_whoop_sync_meta', JSON.stringify(syncMeta));
